...
public class cfunctionpage extends JFrame implements ActionListener{
	...
	//主页选项页面
    private JLabel jl;
	JButton ccreate,cdel,cchange,cquery;
	...
	public cfunctionpage() {
    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\售票员功能页.png");             // 背景图片
	...
	void cfunctionpage1() {
		...
		//添加按钮
		ccreate = new JButton("增    加");
		ccreate.setBounds(160, 180, 100, 50);
		ccreate.addActionListener(this);
		
		//查询按钮
		cdel = new JButton("删    除");
		cdel.setBounds(310, 180, 100, 50);
		cdel.addActionListener(this);
		
		//修改按钮
		cchange = new JButton("修    改");
		cchange.setBounds(160, 250, 100, 50);
		cchange.addActionListener(this);
		
		//查询按钮
		cquery = new JButton("查    询");
		cquery.setBounds(310, 250, 100, 50);
		...
		public void actionPerformed(ActionEvent e) {
	        ...
	        if (e.getSource() == shome) {
	            ...
	            System.exit(0);
	        }else if (e.getSource() == ccreate) {
	        	ccreate.setBackground(Color.green);       // 绿色添加
	            dispose();
	            new ccpage().setBounds(750,50,592,660);
	        }else if (e.getSource() == cdel) {
	        	cdel.setBackground(Color.red);     // 红色删除
	            dispose();
	            new cdpage().setBounds(750,50,592,660);
	        }else if (e.getSource() == cchange) {
	        	cchange.setBackground(Color.getHSBColor(51, 72, 71));     // 灰色修改
	            dispose();
	            new cchpage().setBounds(750,50,592,660);
	        }else if (e.getSource() == cquery) {
	        	cquery.setBackground(Color.yellow);     // 黄色查询
	            dispose();
	            new cqpage().setBounds(750,50,592,660);
	        }else if (e.getSource() == srefresh) {
	            srefresh.setBackground(Color.blue);     // 蓝色刷新
	            dispose();
	        	new cfunctionpage().setBounds(750,50,576,650);
