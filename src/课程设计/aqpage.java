...
public class aqpage extends JFrame  implements ActionListener{
	private static final long serialVersionUID = 1L;
		...
		String[] ccp0 = new String[]{"车辆（Coach）","站点（City）","发车时间（Ctime）","车票（Ticket）","售票员（Conductor）","乘客（Passenger）","退票（Refund）","————  请选择  ————"};
		JButton ccrefresh,cchome,ccback,ccexit;
		Statement sql;
		ResultSet coachrs,cityrs,timers,ticketrs,conductorrs,passengerrs,refundrs;
		...
		//售票员
		JLabel jl4;
		String conductorsql = "use 郑烁彬_sj "+"\n"+" select * from Conductor";
		String [] conductorcolumn;
		String [][] conductorrecord;
		String conductorhead[] = {"工号","姓名","性别"};
		String [] conductorHead;
		String [][] conductorcontent;
		Statement conductorSql;
		JTable conductortable;
		JScrollPane conductorTable ;
		...
		public aqpage() {
	    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\管理员查询数据.png");             // 背景图片
		...
		void conductor0() {
			jl4 = new JLabel();
			jl4.setBounds(0, 0, 576, 562);
			conductor1();

			conductorcontent = conductorrecord;
			conductorHead = conductorcolumn;
			conductortable = new JTable(conductorcontent,conductorHead);
			conductorTable = new JScrollPane(conductortable);
			conductorTable.setBounds(0,150,576,408);
		...
		void conductor1() {
			try {
				Con = DriverManager.getConnection(RL,"sa","5436.......");
				conductorSql = Con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
				conductorrs = conductorSql.executeQuery(conductorsql);
	            ResultSetMetaData conductorData =conductorrs.getMetaData();
	            int conductorcolumnCount = conductorData.getColumnCount();
	            conductorcolumn = new String[conductorcolumnCount];
	            for(int i = 1;i <= conductorcolumnCount;i ++)
	            {
	            	conductorcolumn[i - 1] = conductorhead[i - 1];
	            }
	            conductorrs.last();
	            int conductorrecordAmount = conductorrs.getRow();
	            conductorrecord = new String[conductorrecordAmount][conductorcolumnCount];
	            int i = 0;
	            conductorrs.beforeFirst();
	            while(conductorrs.next()) {
	            	for(int j = 1;j <= conductorcolumnCount;j ++) {
	            		conductorrecord[i][j -1] =conductorrs.getString(j);
	            	}
	            	i ++;
	            }
	            conductorrs.close();
				...
				System.out.println("conductor表" + e + "有误！请输入正确的表名！！");
			}
		}
		...
		public void actionPerformed(ActionEvent e) {
			try {
		        ...
		        if (e.getSource() == ccexit) {
		        	...
		        	dispose();
			        new aqpage().setBounds(750,50,592,660);      // 刷新
		        }
		      //菜单栏事件
