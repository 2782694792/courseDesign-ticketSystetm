...
public class acpage extends JFrame  implements ActionListener{
	...
	//菜单栏
			...
			public acpage() {
		    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\管理员添加数据.png");             // 背景图片
		...
		//乘客组件
				...
				void conductor1() {
			        ...
			        try{ 
			           	Connection con = DriverManager.getConnection(RL,"sa","5436.......");
			           	sm = con.createStatement();
			           	int con2 = sm.executeUpdate("use 郑烁彬_sj"+"\n"+"insert into Conductor select "+"\'"+concno.getText()+"\',"+"\'"+conname.getText()+"\',"+"\'"+consex.getText()+"\';"); 	//读取数据库数据  
			           	if(con2 == 1){
		               	 Object[] options ={"确定"};  //自定义按钮上的文字
		               	 JOptionPane.showOptionDialog(null,
		               			 "———— "+ccp.getSelectedItem()+"添加成功 ————" +"\n\n\t工号："+concno.getText()+"\n\t姓名："+conname.getText()+"\n\t性别："+consex.getText(),
		...
		public void actionPerformed(ActionEvent e) {
			try {
		        ...
		        if (e.getSource() == ccexit) {
		        	...
		        	dispose();
			        new acpage().setBounds(750,50,592,660);      // 刷新
		        }
		      //菜单栏事件
