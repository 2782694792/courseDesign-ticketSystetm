...
public class pquerypage extends JFrame implements ActionListener{
	...
	//主页选项页面
    private JLabel jl;
	private JButton pvehicle,psite,pdeparturetime,pticket;
	
	//主页、退出程序、返回、刷新按钮
	private JButton pghome,pgexit,pgback,pgrefresh;
	
	public pquerypage() {
    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\乘客查询页.png");             // 背景图片
	...
	void pquerypage1() {
		...
		// 设置总体标签容器
		jl = new JLabel();
	    jl.setBounds(0, 0, 592, 660);
		
		//车辆按钮
	    pvehicle = new JButton("车      辆");
	    pvehicle.setBounds(160, 170, 100, 50);
	    pvehicle.addActionListener(this);
	    
	    //站点按钮
	    psite = new JButton("站      点");
	    psite.setBounds(310, 170, 100, 50);
	    psite.addActionListener(this);
		
		//车票按钮
	    pdeparturetime = new JButton("发车时间");
	    pdeparturetime.setBounds(160, 310, 100, 50);
	    pdeparturetime.addActionListener(this);

	    //车票按钮
	    pticket = new JButton("车      票");
	    pticket.setBounds(310, 310, 100, 50);
	    pticket.addActionListener(this);
	    
		//主页按钮
		pghome = new JButton("主页");
		pghome.setBounds(50, 60, 70, 30);
		pghome.addActionListener(this);
		pghome.setForeground(Color.blue);
		
		//退出程序按钮
		pgexit = new JButton("退出");
		pgexit.setBounds(450, 60, 70, 30);
		pgexit.addActionListener(this);
		pgexit.setForeground(Color.red);
		
		//主页按钮
		pgrefresh = new JButton("刷新");
		pgrefresh.setBounds(80, 380, 70, 30);
		pgrefresh.addActionListener(this);
				
		//退出程序按钮
		pgback = new JButton("返回");
		pgback.setBounds(420, 380, 70, 30);
		pgback.addActionListener(this);
		
		jl.add(pvehicle);
		jl.add(psite);
		jl.add(pdeparturetime);
		jl.add(pticket);
		jl.add(pghome);
		jl.add(pgexit);
		jl.add(pgback);
	    ...
	    */
		...
		public void actionPerformed(ActionEvent e) {
	        if (e.getSource() == pghome) {
	            ...
	            System.exit(0);
	        }else if (e.getSource() == pvehicle) {
	        	pvehicle.setBackground(Color.green);       // 绿色车辆
	            new pcoachquery().setBounds(0,50,500,550);
	        }else if (e.getSource() == psite) {
	        	psite.setBackground(Color.blue);       // 蓝色站点
	            new pcityquery().setBounds(0,50,500,550);
	        }else if (e.getSource() == pdeparturetime) {
	        	pdeparturetime.setBackground(Color.BLACK);       // 灰色发车时间
	            new pctimequery().setBounds(0,50,500,550);
	        }else if (e.getSource() == pticket) {
	        	pticket.setBackground(Color.yellow);      // 黄色车票
	        	new ticketquery().setBounds(0,50,500,550);
