...
public class cdpage extends JFrame  implements ActionListener{
	private static final long serialVersionUID = 1L;
		...
		public cdpage() {
	    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\售票员删除数据.png");             // 背景图片
                ...
                String rtime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(date);
	           	...
	           	int tic2 = sm.executeUpdate("use 郑烁彬_sj"+"\n"
							...
							+"Pseat = \'"+passeat.getText()+"\';"
	           				...
	           				+" from Passenger"
	           				+" where Passenger.Tno = "+"\'"+pastno.getText()+"\'"
	           				+" and Passenger.Pname = "+"\'"+pasname.getText()+"\'"
	           				+" and Passenger.Pid = "+"\'"+pasid.getText()+"\'"
	           				+" and Passenger.Pnum	= "+"\'"+pasnum.getText()+"\'"
		...
		void refund1() {
	        ...
	        try{ 
	           	Connection con = DriverManager.getConnection(RL,"sa","5436.......");
	           	sm = con.createStatement();
	           	if(Integer.parseInt(reftprice.getText()) >= 0) {}
	           	int tic2 = sm.executeUpdate("use 郑烁彬_sj"+"\n"
		...
		public void actionPerformed(ActionEvent e) {
			try {
		        ...
		        if (e.getSource() == ccexit) {
		        	...
		        	dispose();
			        new cdpage().setBounds(750,50,592,660);      // 刷新
		        }
		      //菜单栏事件
