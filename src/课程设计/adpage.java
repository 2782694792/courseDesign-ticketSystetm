...
public class adpage extends JFrame  implements ActionListener{
	private static final long serialVersionUID = 1L;
		...
		//售票员
		JLabel jl6,jl61,jl62,jl63;
		JButton concreate,conreset,conreset1,conreset2,conreset3;
		JTextField concno,conname,consex;
		
		public adpage() {
	    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\管理员删除数据.png");             // 背景图片
		...
		void conductor0() {
			...
			//车次输入框
			jl61 = new JLabel("工       号：");
			jl61.setBounds(100, 160, 70, 20);
			concno = new JTextField(10);
			concno.setBounds(200, 160, 165, 20);
			//车次清空按钮
			conreset1 = new JButton("清空");
			conreset1.setBounds(400, 160, 70, 20);
			...
			//座位数输入框
		    jl62 = new JLabel("姓       名：");
		    jl62.setBounds(100, 190, 70, 20);
		    conname = new JTextField(20);
		    conname.setBounds(200, 190, 165, 20);
			//座位数清空按钮
		    conreset2 = new JButton("清空");
		    conreset2.setBounds(400, 190, 70, 20);
			...
			//密码输入框
		    jl63 = new JLabel("性       别：");
		    jl63.setBounds(100, 220, 70, 20);
		    consex = new JTextField(20);
		    consex.setBounds(200, 220, 165, 20);
			//密码清空按钮
		    conreset3 = new JButton("清空");
		    conreset3.setBounds(400, 220, 70, 20);
		...
		void conductor1() {
			coacreate.setBackground(Color.blue);      // 蓝色提交
            if (concno.getText().equals("") == true && conname.getText().equals("") == true && consex.getText().equals("") == true ) {
            	JOptionPane.showMessageDialog(null, "———— 必填项不可为空 ————", "操作有误", JOptionPane.WARNING_MESSAGE);
			} else if (concno.getText().equals("") == false && conname.getText().equals("") == false && consex.getText().equals("") == true ) {
				JOptionPane.showMessageDialog(null, "———— 性别不可为空 ————", "操作有误", JOptionPane.ERROR_MESSAGE);
				consex.requestFocusInWindow();
			} else if (concno.getText().equals("") == false && conname.getText().equals("") == true && consex.getText().equals("") == false ) {
				JOptionPane.showMessageDialog(null, "———— 姓名不可为空 ————", "操作有误", JOptionPane.ERROR_MESSAGE);
				conname.requestFocusInWindow();
			}  else if (concno.getText().equals("") == true && conname.getText().equals("") == false && consex.getText().equals("") == false ) {
				JOptionPane.showMessageDialog(null, "———— 工号不可为空 ————", "操作有误", JOptionPane.ERROR_MESSAGE);
				concno.requestFocusInWindow();
			} else if (concno.getText().equals("") == false && conname.getText().equals("") == false && consex.getText().equals("") == false ) {
			Statement sm = null;
	        try{ 
	           	Connection con = DriverManager.getConnection(RL,"sa","5436.......");
	           	sm = con.createStatement();
	           	int coa2 = sm.executeUpdate("use 郑烁彬_sj"+"\n"
	           				+"delete from Conductor where "
							+"Cno = \'"+concno.getText()+"\' and "
		...
		public void actionPerformed(ActionEvent e) {
			try {
		        ...
		        if (e.getSource() == ccexit) {
		        	...
		        	dispose();
			        new adpage().setBounds(750,50,592,660);      // 刷新
		        }
		      //菜单栏事件
