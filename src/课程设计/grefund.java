...
public class grefund extends JFrame implements ActionListener{
	...
	public grefund() {
    	background = new ImageIcon("D:\\eclipse\\课程设计\\bin\\img\\团队退票.png");             // 背景图片
	        ...
	        if (e.getSource() == psubmit) {
	            ...
	            //监听文本框输入事件
		            if (tno.getText().equals("") == true && name.getText().equals("") == false && id.getText().equals("") == false && num.getText().equals("") == false) {
		            	...
		            	JOptionPane.showMessageDialog(null, "———— 必填项不可为空 ————", "操作结果", JOptionPane.OK_OPTION);
			            psubmit.requestFocusInWindow();
			            //JOptionPane.showConfirmDialog(null, "———— 必填项不可为空 ————", null, JOptionPane.OK_OPTION);
		            }else {
		            	JOptionPane.showMessageDialog(null, "———— 输入有误！请重新输入————", "操作结果", JOptionPane.OK_OPTION);
			            psubmit.requestFocusInWindow();
	            ...
	            dispose();
	            new grefund().setBounds(750,50,576,650);      // 刷新
	            ...
	            dispose();
	        	new porgr().setBounds(750,50,592,660);
